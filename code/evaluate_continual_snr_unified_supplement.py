...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "results" / "continual_snr_unified_supplement_72_96_30_40"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Supplement continual few-shot SNR robustness results with full unified metrics "
            "for selected support sizes and SNR levels."
        )
    )
    ...
    parser.add_argument(
        "--snr-results-dir",
        type=Path,
        default=metric_eval.DEFAULT_SNR_RESULTS,
        help="Existing SNR result directory used for fold_test_ids/run provenance.",
    )
    parser.add_argument(
        "--snr-base-results-dir",
        type=Path,
        default=None,
        help="Base model directory. Defaults to --optimized-results-dir.",
    )
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--support-list", type=str, default="72,96")
...
def main():
    ...
    args.snr_results_dir = resolve_path(args.snr_results_dir)
    ...
    run_config["mode"] = "snr"
    run_config["note"] = (
        "SNR robustness unified metric supplement. This recomputes detailed waveform, "
        "spectral convergence, envelope PCC, and heart-rate metrics for the requested "
        "support/SNR conditions."
    )
    (args.output_dir / "run_config.json").write_text(json.dumps(run_config, indent=2), encoding="utf-8")

    result = metric_eval.run_snr(args, subjects, arrays, device)
    metric_eval.write_csv(args.output_dir / "requested_snr_unified_metrics.csv", result["requested"])
    (args.output_dir / "summary.json").write_text(
        json.dumps({"continual_snr_robustness": result}, indent=2), encoding="utf-8"
    )
    print(f"Saved SNR unified metric supplement to: {args.output_dir / 'requested_snr_unified_metrics.csv'}")
